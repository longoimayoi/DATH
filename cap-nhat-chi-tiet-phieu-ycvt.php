...
if(isset($_POST['Back']))
{
	...
	if($SL <=0)
	{
	   // $query_up="UPDATE tblhoadon SET TrangThai=1 WHERE MaHD=".$_GET['MaHD']." ";
	    $query_up="UPDATE tblhoadon SET TrangThai=1,idNVPQT=".$_SESSION['uid'].",NgayNVPQTDuyet='$today', TongTien = (SELECT SUM(ThanhTien) as ThanhTien from tblphieuyeucautrangbi WHERE MaHD='{$_GET['MaHD']}') WHERE MaHD=".$_GET['MaHD'];
...
<body>  
	...
	$(document).on('click', '.check_box', function(){
					...
					if(this.checked)
					{
						html = '<td><input type="checkbox" STT="'+$(this).attr('stt')+'" data-MaVatTu="'+$(this).data('mavattu')+'" data-TenVatTu="'+$(this).data('tenvattu')+'" data-DVT="'+$(this).data('dvt')+'" data-SL="'+$(this).data('sl')+'" data-GoiYDG="'+$(this).data('goiydg')+'" data-DonGia="'+$(this).data('dongia')+'" data-ThanhTien="'+$(this).data('thanhtien')+'" data-GhiChu="'+$(this).data('ghichu')+'" class="check_box" checked /></td>';
						html += '<td><input disabled type="text" name="MaVatTu[]" class="form-control" value="'+$(this).data("mavattu")+'" /></td>';
						...
						html += '<td><input disabled type="text" name="GoiYDG[]" class="form-control" value="'+$(this).data("goiydg")+'" /></td>';
					...
					{
						html = '<td><input type="checkbox" STT="'+$(this).attr('stt')+'" data-MaVatTu="'+$(this).data('mavattu')+'" data-TenVatTu="'+$(this).data('tenvattu')+'" data-DVT="'+$(this).data('dvt')+'" data-SL="'+$(this).data('sl')+'" data-GoiYDG="'+$(this).data('goiydg')+'" data-DonGia="'+$(this).data('dongia')+'" data-ThanhTien="'+$(this).data('thanhtien')+'" data-GhiChu="'+$(this).data('ghichu')+'" class="check_box" /></td>';
						html += '<td>'+$(this).data('mavattu')+'</td>';
						html += '<td>'+$(this).data('tenvattu')+'</td>';
						html += '<td>'+$(this).data('dvt')+'</td>';
						html += '<td>'+$(this).data('sl')+'</td>';
						html += '<td>'+$(this).data('goiydg')+'</td>';
				...
				function fetch_data()
				{
					...
					$.ajax({
						...
						success:function(data)
						{
							...
							{
								html += '<tr>';
								html += '<td><input type="checkbox" STT="'+data[count].STT+'"data-MaVatTu="'+data[count].MaVatTu+'"data-TenVatTu="'+data[count].TenVatTu+'" data-DVT="'+data[count].DVT+'" data-SL="'+data[count].SL+'" data-GoiYDG="'+data[count].GoiYDG+'" data-DonGia="'+data[count].DonGia+'" data-ThanhTien="'+data[count].ThanhTien+'"data-GhiChu="'+data[count].GhiChu+'" class="check_box"  /></td>';
								html += '<td>'+data[count].MaVatTu+'</td>';
								html += '<td>'+data[count].TenVatTu+'</td>';
								html += '<td>'+data[count].DVT+'</td>';
								html += '<td>'+data[count].SL+'</td>';
								html += '<td>'+data[count].GoiYDG+'</td>';
