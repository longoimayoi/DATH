      ...
      <div class="card">
        ...
        <div class="card-body card-block">
          <form action="" method="post">
            ...
            <div class="form-group  col-lg-6">
              ...
              <?php 
              $sqlTTP = "SELECT * FROM tblhoadon WHERE MaHD=".$MaHD;
              $queryTTP = mysqli_query($connect, $sqlTTP);
              $rowTTP = mysqli_fetch_assoc($queryTTP);
              if($rowTTP['MaKhoa']!="")
              {
               $query="SELECT TenKhoa FROM tblkhoa where MaKhoa=".$rowTTP['MaKhoa'];
               $result=mysqli_query($connect,$query);
               $item1= mysqli_fetch_assoc($result); 
               ?>
               <input disabled="" class="form-control" value="<?php echo $item1['TenKhoa'] ?>" >   
               <?php
             }
             else
             {
          ...
          <div class="form-group  col-lg-6 edit">
            ...
            <?php  }}if($TrangThai==10) {?>
              ...
              <?php  }if($TrangThai==11) {?>
                <span style="background-color: #6ecf72; width: auto;height: auto"class="badge badge-pill badge-dark">Đã tổng hợp</span>
                <div style="float:right;">  
                </div>
              <?php  }if($TrangThai==12) {?>
                <span style="background-color: #debfe8;width: auto;height: auto" class="badge badge-pill badge-dark">Đã nhập kho</span>
                <div style="float:right;">  
                </div>
              <?php } ?>
            </div>
        ...
        //echo number_format($tien2);
          ...
          <?php } elseif($TrangThai ==5) {?>
            <div>
             <div class="submit" style="float: left;margin-right: 1px">
              <a href = "template/file-phieu-yeu-cau-trang-bi.xml" download><button type="submit" name="taiFileMau">Tải file mẫu</button></a>
              <form name='import' method="POST" enctype="multipart/form-data">
                <button  style="background-color: #217346" type="submit" name="file" name="import"class="addfiles"><i class="ti-upload"></i> Import</button>
                <input data-toggle="collapse" href="#collapse" class="collapsed" id="file-upload" type="file" name="file" multiple style='display: none;'>
                <button id="collapse" class="panel-collapse collapse" type="submit" name="import">Nhập excel</button>
